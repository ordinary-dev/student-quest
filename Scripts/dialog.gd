...
func hide_dialog():
	get_node("/root/Node2D").remove_child(get_node("/root/Node2D/dialog"))

func show_next(content : Dictionary, key : String) -> void:
	var name : = get_node("/root/Node2D/dialog/Dialog/Main/Margin/HBox/VBox/Name")
	var text : = get_node("/root/Node2D/dialog/Dialog/Main/Margin/HBox/VBox/Text")
	var btn : = get_node("/root/Node2D/dialog/Dialog/Main/Margin/HBox/Button")
	...
	get_node("/root/Node2D/dialog/AnimationPlayer").play("text")
	

func show_dialog(num : int) -> void:
	if has_node("/root/Node2D"):
		# Подготовить интерфейс
		var nd : = get_node("/root/Node2D")
